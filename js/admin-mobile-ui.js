function initAdminMobileUi(){
  ...
  const style=document.createElement('style');style.id='admin-mobile-ui-styles';style.textContent=`
    .admin-collapsible{position:relative}.admin-collapsible>h2{cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:10px;user-select:none;margin-bottom:0!important}.admin-collapsible>h2::after{content:'−';font-size:1rem;font-weight:700;color:#6b7280;transition:transform .18s ease}.admin-collapsible.is-collapsed>h2::after{content:'+';transform:none}.admin-collapsible.is-collapsed>*:not(h2){display:none!important}
