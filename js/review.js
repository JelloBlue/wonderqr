import { SUPABASE_URL, SUPABASE_ANON_KEY, supabase } from './config.js?v=8';
...
function normalizePhone(value){let digits=String(value||'').replace(/\D/g,'');if(digits.length===10)digits='91'+digits;if(digits.length<10)return '';return `tel:+${digits}`;}function setupSocialLinks(){const socialBlock=document.getElementById('social-block');if(!socialBlock)return;const links=[['link-insta','instagram_url',v=>v],['link-yt','youtube_url',v=>v],['link-fb','facebook_url',v=>v],['link-wa','whatsapp_number',v=>`https://wa.me/${String(v).replace(/\D/g,'')}`],['link-call','phone_number',normalizePhone],['link-justdial','justdial_url',v=>v],['link-pinterest','pinterest_url',v=>v],['link-x','x_url',v=>v]];let hasSocial=false;links.forEach(([id,field,toHref])=>{const e=document.getElementById(id);if(e)e.classList.add('hidden');const value=String(currentBusiness?.[field]||'').trim();if(value&&e){const href=toHref(value);if(!href)return;e.href=href;e.target='_self';e.addEventListener('click',event=>{if(e.id==='link-call'&&e.href.startsWith('tel:')){event.preventDefault();window.location.href=e.href;}});e.classList.remove('hidden');hasSocial=true;}});if(hasSocial)socialBlock.classList.remove('hidden');else socialBlock.classList.add('hidden');}
