...
async function openOwnerAdmin(){const pin=window.prompt('Enter 4-digit Admin PIN');if(pin===null)return;if(!/^\d{4}$/.test(pin)){alert('Please enter exactly 4 digits.');return;}try{const response=await fetch(`${supabase.supabaseUrl}/functions/v1/verify_admin_pin`,{method:'POST',headers:{'Content-Type':'application/json','apikey':supabase.supabaseKey},body:JSON.stringify({business_id:currentBusiness.id,pin})});const result=await response.json();if(!response.ok||!result.ok||!result.owner_token){alert('Incorrect Admin PIN.');return;}window.location.href=`${window.location.origin}/wonderqr/admin.html?token=${encodeURIComponent(result.owner_token)}`;}catch(error){console.error('Admin PIN verification failed:',error);alert('Unable to verify PIN. Please try again.');}}
