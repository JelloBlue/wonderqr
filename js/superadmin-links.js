...
(() => {
  const fields = [
    ['facebook_url', 'Facebook Page URL'], ['whatsapp_number', 'WhatsApp Number'], ['phone_number', 'Phone Number'],
    ['justdial_url', 'Justdial URL'], ['zomato_url', 'Zomato URL'], ['swiggy_url', 'Swiggy URL'],
    ['pinterest_url', 'Pinterest URL'], ['x_url', 'X (Twitter) Profile URL']
  ];
  ...
  function addFields(formId, prefix) {
    ...
    const box = document.createElement('div'); box.dataset.socialFields = 'true';
    ...
    if (anchor?.parentNode) anchor.parentNode.insertBefore(box, anchor.nextSibling); else form.insertBefore(box, form.querySelector('button[type="submit"]'));
    form.dataset.socialFieldsReady = 'true';
  }
  function validate(prefix) {
    for (const [id,label] of fields) { const el=document.getElementById(`${prefix}-${id}`); if(!el) continue; const v=el.value.trim();
  ...
  }}catch(e){console.warn('Social link form enhancement:',e);}return originalFetch(input,init);};

  let wrapped=false;
  const tryWrap=()=>{if(wrapped||typeof window.openEditModal!=='function')return;const original=window.openEditModal;window.openEditModal=function(id){original(id);setTimeout(()=>{const b=(window.loadedBusinesses||[]).find(x=>String(x.id)===String(id));if(!b)return;fields.forEach(([name])=>{const el=document.getElementById(`edit-${name}`);if(el)el.value=b[name]||'';});},0);};wrapped=true;};
  const timer=setInterval(()=>{tryWrap();if(wrapped)clearInterval(timer);},100);
})();
